...
def get_dataset_name(args=None):
    if args.dataset == "Split-DailySports":
        return "DailySports"
...
def get_descriptors_embedding(class_mask, task_id, args=None):
    k = args.num_prompts_per_task
    ...
    print("generate descriptors embedding for classes:")
    print(class_mask[task_id])
    for item in class_mask[task_id]:
        attribs = list(desc.items())[item][1]
        print("class : "+str(item))
        print(attribs)
        # desc_list = getUnion(desc_list, attribs)
        ...
        encodings = model.encode(attribs, convert_to_tensor=True)
...
def get_descriptors_embedding1(class_mask, task_id, args=None):
    k = args.num_prompts_per_task
    ...
    dataset_name = get_dataset_name(args)
    # path = os.getcwd()+"/descriptors/descriptors_"+dataset_name+".json"
    # path = os.getcwd()+"/descriptors/descriptors_"+dataset_name+"_gpt4.json"
    # path = os.getcwd()+"/descriptors/descriptors_"+dataset_name+"_gpt4o.json"
    # path = os.getcwd()+"/descriptors/descriptors_"+dataset_name+"_gpt4o2.json"
    # if "CIFAR" in args.dataset:

    if "Imagenet-R" in args.dataset:
        path = os.getcwd()+"/descriptors/descriptors_"+dataset_name+"_gptlong.json"
        # path = os.getcwd()+"/descriptors/descriptors_"+dataset_name+".json"
    if "Imagenet-A" in args.dataset:
        path = os.getcwd()+"/descriptors/descriptors_"+dataset_name+"_nameonly.json"
    elif "CUB" in args.dataset:
        # path = os.getcwd()+"/descriptors/descriptors_"+dataset_name+"_gpt4q.json"
        path = os.getcwd()+"/descriptors/descriptors_"+dataset_name+"_gpt.json"
        # path = os.getcwd()+"/descriptors/descriptors_"+dataset_name+".json"
    else:
        path = os.getcwd()+"/descriptors/descriptors_"+dataset_name+"_gpt.json"
        # path = os.getcwd()+"/descriptors/descriptors_"+dataset_name+"_gpt4q.json"
    ...
    if task_id == 0:
        print("All classes names : ", list(desc.keys()))
        # print("All classes names")
        # for task_id in range(args.num_tasks):
        #     class_name =  [str(list(desc.items())[i][0]) for i in class_mask[task_id] ]
        #     print(class_name)
    ...
    for item in class_mask[task_id]:
        ...
        for i in range(1,len(attribs)):
            att = att + "," +attribs[i]
        print("class : "+str(item)+", class_name: " + str(list(desc.items())[item][0]))
        print(att)
        # desc_list = getUnion(desc_list, attribs)
