...
def get_dataset(dataset, transform_train, transform_val, args,):
    if dataset == 'CIFAR100':
        ...
        dataset_train = Imagenet_A(args.data_path, train=True, download=True, transform=transform_train).data
        dataset_val = Imagenet_A(args.data_path, train=False, download=True, transform=transform_val).data
        dataset_feat_train = Imagenet_A(args.data_path, train=True, download=True, transform=transform_val).data
    elif dataset == "DailySports":
        dataset_train = DailySports(root='local_datasets', train=True)
        dataset_val = DailySports(root='local_datasets', train=False)
        dataset_feat_train = dataset_train
...
def build_transform(is_train, args):
    ...
    if is_train:
        if hasattr(args, "mean") and hasattr(args, "std"):
            transforms_list.append(lambda x: (x - args.mean) / args.std)
        else:
            transforms_list.append(lambda x: x)

        transforms_list.append(lambda x: x + 0.01*torch.randn_like(x))
        transforms_list.append(lambda x: x.flip(0) if torch.rand(1) < 0.5 else x)
    else:
        if hasattr(args, "mean") and hasattr(args, "std"):
            transforms_list.append(lambda x: (x - args.mean) / args.std)
        else:
