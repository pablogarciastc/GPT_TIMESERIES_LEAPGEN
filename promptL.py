...
class LPrompt(nn.Module):
    def __init__(self, length=5, embed_dim=768, num_tasks=10, num_classes=100, embedding_key='mean',
                 prompt_init='uniform', prompt_pool=False,
                 prompt_key=False, pool_size=None, top_k=1, top_k_l=3, batchwise_prompt=False,
        ...
        print("Num Tasks: ", self.n_tasks, "pool_size: ", self.pool_size, "num_classes: ", num_classes, "top_k: ",
              top_k)
    ...
    def forward(self, x_embed, y, task_id=-1, prompt_mask=None, layer_num=-1, cls_features=None):
        ...
        if self.prompt_pool:
            ...
            prompt_key2_norm_allclass = torch.flatten(
                (prompt_key2_norm.unsqueeze(1).repeat(1, self.numclasses_per_task, 1)), start_dim=0, end_dim=1)
            # print("prompt_key2_norm_allclass shape: ",prompt_key2_norm_allclass.shape)

            sim = torch.matmul(prompt_key_norm, x_embed_norm.t())
            sim = sim.t()
            sim2 = torch.matmul(prompt_key2_norm_allclass, x_embed_norm.t())  # pool_size, B or Pool_size, #class, B
            ...
            if self.use_prefix_tune_for_e_prompt:
                ...
                batched_prompt_raw = batched_prompt_raw.view(x_embed.shape[0], self.num_heads,
                                                             self.embed_dim // self.num_heads).unsqueeze(0)
                batched_prompt_raw = torch.cat((batched_prompt_raw, batched_prompt_raw),
                                               dim=0)  # 2 x B x n_head x dim/n_head
