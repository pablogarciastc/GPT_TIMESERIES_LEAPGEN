...
class VisionTransformerL(nn.Module):
    ...
    def __init__(
            ...
            num_tasks=5, kernel_size=17, top_k=None,top_k_l=None, batchwise_prompt=False, prompt_key_init='uniform', head_type='token', use_prompt_mask=False,
        ...
        if use_e_prompt and e_prompt_layer_idx is not None:
            self.e_prompt = LPrompt(length=prompt_length, embed_dim=embed_dim, num_tasks=self.num_tasks, num_classes=num_classes, embedding_key=embedding_key, prompt_init=prompt_init,
                    prompt_pool=prompt_pool, prompt_key=prompt_key, pool_size=pool_size, top_k=top_k, top_k_l=top_k_l, batchwise_prompt=batchwise_prompt,
        ...
        if not self.use_multihead:
            self.head = nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
            # first_head = nn.Linear(self.embed_dim, int(num_classes/num_tasks))
            # self.head = []
            # self.head.append(first_head)

        else:
            self.head = SimpleContinualLinear(embed_dim, (int)(self.num_classes/self.num_tasks))
            # self.head = SimpleContinualLinear(2*embed_dim, (int)(self.num_classes/self.num_tasks))
        
        # self.task_predictor = SimpleContinualLinear(embed_dim, 1)
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False):
        ...
        # print("input x after patching  shape")
        # print(x.shape)
        ...
        # print("input x add with cls token  shape")
        # print(x.shape)
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            if self.use_g_prompt or self.use_e_prompt:
                ...
                last_e_prompt = None
                # print("input x before loop  shape")
                # print(x.shape)
                ...
                for i, block in enumerate(self.blocks):
                    if i in self.g_prompt_layer_idx:
                        ...
                        print("input x to prompt shape")
                        print(x[:, 0].shape)
                        print("check prompt  shape")
                        print(self.e_prompt.prompt.shape)
                        print(self.e_prompt.prompt_key.shape)

                        e_prompt_counter += 1
                        res = self.e_prompt(x, task_id=task_id, prompt_mask=prompt_mask, layer_num=e_prompt_counter, cls_features=x[:, 0])
                        e_prompt = res['batched_prompt']
                        
                        print("check batch e_prompt  shape")
                        ...
                        if self.use_prefix_tune_for_e_prompt:
                            ...
                            print("input ne_prompt")
                            print(ne_prompt.shape)
        ...
        # print("input x after norm shape")
        # print(x.shape)
    ...
    def forward_featuresA1(self, x, y, task_id=-1, cls_features=None, train=False):
        # if y is None:
        #     print("check 1")
        x = self.patch_embed(x)
            ...
            if self.use_g_prompt or self.use_e_prompt:
                ...
                for i, block in enumerate(self.blocks):
                    ...
                    if i in self.e_prompt_layer_idx:
                        ...
                        res = self.e_prompt(x, y, task_id=task_id, prompt_mask=prompt_mask, layer_num=e_prompt_counter, cls_features=cls_features)

                        e_prompt = res['batched_prompt']

                        desc_embed = res['desc_embed'].unsqueeze(1)
    ...
    def forward_headA(self, res, device, pre_logits: bool = False):
        x = res['x']
    ...
    def forward_headA1(self, res, task_id, device, pre_logits: bool = False):
        x = res['x']
    ...
    def forward_head(self, res, device, pre_logits: bool = False):
        ...
        # print("input x to head shape")
        # print(x.shape)
        # print(self.total_prompt_len)
    ...
    def forward(self, x, task_id=-1, cls_features=None, train=False):
        ...
        # res = self.forward_head2(res, device=x.device)
        return res


    def forwardA1(self, x, y, task_id=-1, cls_features=None, train=False, kl_idx=None):
        res = self.forward_featuresA1(x, y, task_id=task_id, cls_features=cls_features, train=train)
...
@torch.no_grad()
def _load_weights(model: VisionTransformerL, checkpoint_path: str, prefix: str = ''):
...
def _create_vision_transformer(variant, pretrained=False, **kwargs):
    ...
    model = build_model_with_cfg(
        VisionTransformerL, variant, pretrained,
...
def vit_base_patch16_224_in21k(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    # model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=6, **kwargs)
