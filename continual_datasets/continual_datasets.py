...
class DailySports(Dataset):
    """DailySports Dataset for time series activity recognition.

    Args:
        root (string): Root directory of dataset where DailySports folder exists.
        train (bool, optional): If True, creates dataset from training set, otherwise
            creates from test set.
        transform (callable, optional): A function/transform that takes in a time series
    ...
    def __init__(
        ...
        # Define class labels (0–17)
        self.classes = [str(i) for i in range(18)]
        self.class_to_idx = {str(i): i for i in range(18)}

        # Load the data
        self.data, self.targets = self._load_data()

    def _load_data(self) -> Tuple[np.ndarray, list]:
        """Load data from pickle files."""
        if self.train:
            x_file = os.path.join(self.root, "DailySports", "x_train.pkl")
            y_file = os.path.join(self.root, "DailySports", "state_train.pkl")
        else:
            x_file = os.path.join(self.root, "DailySports", "x_test.pkl")
            y_file = os.path.join(self.root, "DailySports", "state_test.pkl")
        ...
        # Reshape to [N, seq_length, num_features]
        n_samples = data.shape[0]
        data = data.reshape(n_samples, self.seq_length, self.num_features)

        # Ensure targets are 0-based
        if targets.min() == 1:
            targets = targets - 1
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        time_series, target = self.data[index], self.targets[index]

        # Convert to torch tensors
        time_series = torch.tensor(time_series, dtype=torch.float32)  # [seq_len, num_features]
        ...
        # print("Sample shape:", time_series.shape)
    ...
    def extra_repr(self) -> str:
        ...
        return f"Split: {split}, Shape: ({self.seq_length}, {self.num_features})"
...
class Imagenet_A(torch.utils.data.Dataset):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False):        
        ...
        self.url = 'https://people.eecs.berkeley.edu/~hendrycks/imagenet-a.tar'
        self.filename = 'imagenet-a.tar'
        ...
        if not os.path.exists(os.path.join(root, 'imagenet-a')):
            import tarfile
            tar_ref = tarfile.open(os.path.join(root, self.filename), 'r')
            tar_ref.extractall(root)
            tar_ref.close()
